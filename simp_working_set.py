...
def determine_working_set_config(model_dims, max_seq_len, max_global_batch_tokens, training_config=None, has_embed=True, has_head=True, num_local_layers=None, chunk_size = None, max_gpu_mem_bytes=None, max_host_mem_bytes=None, leeway_gpu_mem_bytes=3e9, leeway_host_mem_bytes=10e9, verbose=False, device_id=0, min_tokens_per_round_limit=None, max_tokens_per_round_limit=None, fixed_seq_len=False, min_chunk_size=None, max_chunk_size=None):
    ...
    target_upper_bound_tokens_per_round = prev_high_div(target_upper_bound_tokens_per_round_est)

    if min_tokens_per_round_limit is not None:
        target_upper_bound_tokens_per_round = max(min_tokens_per_round_limit, target_upper_bound_tokens_per_round)
    ...
    if fixed_seq_len:
        cur_tokens_per_round = round_to_nearest(cur_tokens_per_round, max_seq_len)
        if cur_tokens_per_round > max_tokens_per_round:
            cur_tokens_per_round -= max_seq_len
            if cur_tokens_per_round > max_tokens_per_round or cur_tokens_per_round == 0:
                raise ValueError(f"Error: Could not find a valid configuration for fixed seq len {max_seq_len}; estimated max tokens per round to be {cur_tokens_per_round}")
