...
extern "C" {
    ...
    int set_flash3_fwd_params(Flash_fwd_params &params,
        ...
        FlashDtype flash_dt = (FlashDtype) flash_dtype_as_int;

        if (flash_dt == FP32){
            params.is_fp32 = true;
        }
        else if (flash_dt == BF16){
            params.is_bf16 = true;
        }
        else if (flash_dt == FP8E4M3){
            params.is_e4m3 = true;
        }
        else{
            if (flash_dt != FP16){
                fprintf(stderr, "Error: dtype of FlashDtype enum val of %d not supported in flash3...\n", flash_dtype_as_int);
            return -1;
            }
        }
    ...
    int flash3_get_fwd_workspace_size(int flash_dtype_as_int, int arch, int num_sm, 
        ...
        ret = set_flash3_fwd_params(params,
                                    arch, num_sm,
                                    flash_dtype_as_int,
                                    max_seqs_in_chunk, max_chunk_size, MY_MAX(max_chunk_size, max_seq_len),
                                    &dummy_ptr, &dummy_ptr, MY_MIN(max_chunk_size, max_seq_len),
    ...
    int flash3_get_bwd_workspace_size(int flash_dtype_as_int, int arch, int num_sm, 
        ...
        FlashDtype dtype = (FlashDtype) flash_dtype_as_int;
    ...
    int flash3_bwd_wrapper(CUstream stream, int arch, int num_sm,
        ...
        FlashDtype flash_dt = (FlashDtype) flash_dtype_as_int;
        if ((flash_dt != FP16) && (flash_dt != BF16)){
